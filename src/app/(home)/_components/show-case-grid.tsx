...
export function ShowcaseGrid() {
  ...
  const { data: featuredEvents, isLoading } = api.event.getFeatured.useQuery();
  ...
  const showcases: ShowcaseItem[] =
    featuredEvents?.map((event) => ({
      title: event.title,
      image: event.image ?? "/images/image_services_grid_1.jpg",
      href: "#",
      type: "video" as const,
    })) ?? [];
  ...
  if (!featuredEvents?.length) {
    return (
      <section className="flex h-96 items-center justify-center bg-stone-900">
