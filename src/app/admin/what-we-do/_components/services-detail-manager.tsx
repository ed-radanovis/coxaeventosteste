...
function SortableServiceDetailItem({
  item,
  onDelete,
  onToggleActive,
  onEdit,
  ...
  const getIconComponent = (iconName: string) => {
    ...
    return iconMap[iconName] ?? <Video className="h-5 w-5" />;
  };
...
export function ServicesDetailManager() {
  ...
  // get all servicedetails from the DB
  const {
    data: serviceDetails,
    isLoading,
    refetch,
  } = api.adminServiceDetail.getAllServiceDetails.useQuery();

  // mutation to reorder
  const reorderMutation =
    api.adminServiceDetail.reorderServiceDetails.useMutation({
      onSuccess: () => refetch(),
    });

  // other mutations
  const deleteMutation = api.adminServiceDetail.deleteServiceDetail.useMutation(
    {
      onSuccess: () => refetch(),
    },
  );
  ...
  const toggleActiveMutation =
    api.adminServiceDetail.toggleServiceDetail.useMutation({
      onSuccess: () => refetch(),
    });
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!over || active.id === over.id || !serviceDetails) return;

    const oldIndex = serviceDetails.findIndex((item) => item.id === active.id);
    const newIndex = serviceDetails.findIndex((item) => item.id === over.id);

    if (oldIndex === -1 || newIndex === -1) return;

    // reorder locally
    const reorderedItems = arrayMove(serviceDetails, oldIndex, newIndex);
  ...
  const handleDeleteServiceDetail = (id: string) => {
    if (confirm("Tem certeza que deseja excluir essa especialidade?")) {
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/what-we-do/services-detail/${id}`);
  };
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col justify-between md:flex-row md:items-center">
          <div>
            <h1 className="flex justify-center text-3xl font-bold text-stone-900 md:justify-start dark:text-stone-100">
              Gerenciar - Nossas Especialidades
            </h1>
          ...
          <Button
            onClick={() => {
              handleTap("new-service-detail");
              router.push("/admin/what-we-do/services-detail/new");
            }}
            onTouchStart={() => handleTap("new-service-detail")}
            className={`mt-4 border transition-all duration-300 ease-in-out md:mt-0 ${
              tappedElement === "new-service-detail"
                ? "scale-95"
                : theme === "dark"
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Buscar serviços por título, descrição ou features..."
                  value={searchTerm}
            ...
            <div className="flex items-center justify-center gap-20 text-sm text-stone-600 md:gap-4 dark:text-stone-400">
              <span>Total: {serviceDetails?.length ?? 0}</span>
              <span>
                Ativos: {serviceDetails?.filter((e) => e.isActive).length ?? 0}
              </span>
            </div>
          </div>
        </CardContent>
      ...
      >
        <SortableContext
          items={serviceDetails?.map((item) => item.id) ?? []}
          strategy={verticalListSortingStrategy}
        >
          <div className="space-y-4">
            {filteredServiceDetails && filteredServiceDetails.length > 0 ? (
              filteredServiceDetails.map((item) => (
                <SortableServiceDetailItem
                  key={item.id}
                  item={item}
              ...
              <Card className="border border-stone-200 bg-stone-50 dark:border-stone-700 dark:bg-stone-800">
                <CardContent className="p-12 text-center">
                  <ImageIcon className="mx-auto mb-4 h-16 w-16 text-stone-400" />
