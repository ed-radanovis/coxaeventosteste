...
function SortableProcessStepItem({
  ...
  const getIconComponent = (iconName: string) => {
    return iconMap[iconName] ?? <SearchIcon className="h-5 w-5" />;
  };
      ...
      <Card className="border border-stone-200 bg-stone-100 py-2 transition-all duration-300 hover:shadow-lg md:py-4 dark:border-stone-700 dark:bg-stone-700/50">
        <CardContent className="p-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-4">
            ...
            {/* stage number */}
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-stone-200 text-lg font-bold text-stone-700 dark:bg-stone-600 dark:text-stone-300">
              {index + 1}
            </div>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0 flex-1">
                  <h3 className="mb-2 text-center text-lg font-semibold break-words text-stone-900 md:text-start dark:text-stone-100">
                    {item.title}
                  </h3>
...
export function ProcessManager() {
  ...
  // get all process steps from the DB
  const {
    data: processSteps,
    isLoading,
    refetch,
  } = api.adminProcess.getAllProcessSteps.useQuery();

  // mutation to reorder
  const reorderMutation = api.adminProcess.reorderProcessSteps.useMutation({
    onSuccess: () => refetch(),
  });

  // other mutations
  const deleteMutation = api.adminProcess.deleteProcessStep.useMutation({
    onSuccess: () => refetch(),
  });

  const toggleActiveMutation = api.adminProcess.toggleProcessStep.useMutation({
    onSuccess: () => refetch(),
  });
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!over || active.id === over.id || !processSteps) return;

    const oldIndex = processSteps.findIndex((item) => item.id === active.id);
    const newIndex = processSteps.findIndex((item) => item.id === over.id);

    if (oldIndex === -1 || newIndex === -1) return;

    // reorder locally
    const reorderedItems = arrayMove(processSteps, oldIndex, newIndex);
  ...
  const filteredProcessSteps = processSteps?.filter(
    (item) =>
  ...
  const handleDeleteProcessStep = (id: string) => {
    if (confirm("Tem certeza que deseja excluir esta etapa do processo?")) {
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/what-we-do/process/${id}`);
  };
    ...
    >
      ...
      <div className="mb-4">
        <div className="flex flex-col justify-between md:flex-row md:items-center">
          ...
          <Button
            onClick={() => {
              handleTap("new-process-step");
              router.push("/admin/what-we-do/process/new");
            }}
            onTouchStart={() => handleTap("new-process-step")}
            className={`mt-4 border transition-all duration-300 ease-in-out md:mt-0 ${
              tappedElement === "new-process-step"
                ? "scale-95"
                : theme === "dark"
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Buscar etapas por título, descrição ou ícone..."
                  value={searchTerm}
            ...
            <div className="flex items-center justify-center gap-20 text-sm text-stone-600 md:gap-4 dark:text-stone-400">
              <span>Total: {processSteps?.length ?? 0}</span>
              <span>
                Ativos: {processSteps?.filter((e) => e.isActive).length ?? 0}
              </span>
            </div>
          </div>
        </CardContent>
      ...
      >
        <SortableContext
          items={processSteps?.map((item) => item.id) ?? []}
          strategy={verticalListSortingStrategy}
        >
          <div className="space-y-4">
            {filteredProcessSteps && filteredProcessSteps.length > 0 ? (
              filteredProcessSteps.map((item, index) => (
              ...
              <Card className="border border-stone-200 bg-stone-50 dark:border-stone-700 dark:bg-stone-800">
                <CardContent className="p-12 text-center">
                  <div className="mx-auto mb-4 h-16 w-16 text-4xl">📋</div>
                  ...
                  <p className="mb-6 text-stone-600 dark:text-stone-400">
                    {searchTerm
                      ...
                      : "Comece criando sua primeira etapa do processo."}
                  </p>
                  <Button
                    onClick={() => router.push("/admin/what-we-do/process/new")}
                  ...
                  >
                    <Plus className="mr-2 h-4 w-4" />
                    Criar Primeiro Processo Criativo
                  </Button>
                </CardContent>
              </Card>
            )}
