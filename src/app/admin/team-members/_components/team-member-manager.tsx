              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0 flex-1">
                  <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-center">
                    <h3 className="text-center text-lg font-semibold break-words text-stone-900 md:text-start dark:text-stone-100">
                      {item.name}
                    </h3>
                    <Badge className="mx-auto w-fit md:mx-0" variant="outline">
                      {item.role}
                    </Badge>
                  </div>

                  <p className="mb-2 line-clamp-2 text-sm break-words text-stone-600 dark:text-stone-400">
                    {item.description}
                  </p>
                  ...
                  {item.expertise.length > 0 && (
                    <div className="mb-2 flex flex-wrap gap-1">
                      {item.expertise.slice(0, 3).map((skill, idx) => (
                        <span
                          key={idx}
                          className="rounded-sm border border-stone-300 bg-stone-200 px-2 py-0.5 text-xs text-stone-700 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-300"
                        >
                          {skill}
                        </span>
                      ))}
                      {item.expertise.length > 3 && (
                        <span className="rounded-sm border border-stone-300 bg-stone-200 px-2 py-0.5 text-xs text-stone-700 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-300">
                          +{item.expertise.length - 3}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {item.linkedin && (
                      <a
                        ...
                        className="rounded-full bg-stone-200 p-1.5 text-blue-600 hover:bg-blue-100 dark:bg-stone-800 dark:text-blue-400 dark:hover:bg-blue-900/30"
                    ...
                    {item.instagram && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="rounded-full bg-stone-200 p-1.5 text-pink-600 hover:bg-pink-100 dark:bg-stone-800 dark:text-pink-400 dark:hover:bg-pink-900/30"
                        onClick={(e) => e.stopPropagation()}
                      >
                    ...
                    {item.youtube && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="rounded-full bg-stone-200 p-1.5 text-red-600 hover:bg-red-100 dark:bg-stone-800 dark:text-red-400 dark:hover:bg-red-900/30"
                        onClick={(e) => e.stopPropagation()}
                      >
                    ...
                    {item.email && (
                      <a
                        ...
                        className="text-carrot-600 hover:bg-carrot-100 dark:text-carrot-400 dark:hover:bg-carrot-900/30 rounded-full bg-stone-200 p-1.5 dark:bg-stone-800"
                        onClick={(e) => e.stopPropagation()}
                      >
...
export function TeamMemberManager() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = api.adminTeamMember.getAllTeamMembers.useQuery();

  // mutation to reorder
  const reorderMutation = api.adminTeamMember.reorderTeamMembers.useMutation({
    onSuccess: () => refetch(),
  });

  // other mutations
  const deleteMutation = api.adminTeamMember.deleteTeamMember.useMutation({
    onSuccess: () => refetch(),
  });

  const toggleActiveMutation = api.adminTeamMember.toggleTeamMember.useMutation(
    {
      onSuccess: () => refetch(),
    },
  );
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!over || active.id === over.id || !teamMembers) return;
    ...
    // reorder locally
    const reorderedItems = arrayMove(teamMembers, oldIndex, newIndex);
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/team-members/${id}`);
  };
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Buscar membros por nome, cargo, descrição ou especialidade..."
                  value={searchTerm}
            ...
            <div className="flex items-center justify-center gap-20 text-sm text-stone-600 md:gap-4 dark:text-stone-400">
              <span>Total: {teamMembers?.length ?? 0}</span>
              <span>
                Ativos: {teamMembers?.filter((e) => e.isActive).length ?? 0}
              </span>
            </div>
          </div>
        </CardContent>
            ...
            {filteredTeamMembers && filteredTeamMembers.length > 0 ? (
              ...
              <Card className="border border-stone-200 bg-stone-50 dark:border-stone-700 dark:bg-stone-800">
                <CardContent className="p-12 text-center">
                  <User className="mx-auto mb-4 h-16 w-16 text-stone-400" />
